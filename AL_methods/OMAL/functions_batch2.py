...
def qbc(committee, unlabeled_data):
    ...
    # res = get_variance(vote_prediction)
    res = np.var(vote_prediction, axis=1)
    # variance_sorted = -np.sort(-res)
...
def error_reduct_fuc(X_train, Pool_X, committee, learning_state_features, loss_reduction_target, remaining_samples, predictor):
    ...
    # model_params = np.tile(model_params,(Pool_X.shape[0],1))
    LALfeatures = unlabeled_data # np.concatenate((model_params, unlabeled_data), axis=1)
